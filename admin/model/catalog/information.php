...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', template = '" . $this->db->escape($data['template']) . "'");
	...
	public function editInformation($information_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', template = '" . $this->db->escape($data['template']) . "' WHERE information_id = '" . (int)$information_id . "'");
	...
	public function getInformations($data = array()) {
		if ($data) {
			...
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				...
				$sql .= " ORDER BY id.information_id";
			}
