...
class ModelSettingStore extends Model {
	public function addStore($data) {
		...
		$data['config_name'] = $data['store_config'][$config_language_id]['config_name'];
	...
	public function getStoreConfigs($store_id) {
		$store_config_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE store_id = '" . (int)$store_id . "' AND code = 'config'");

		foreach ($query->rows as $result) {
			$store_config_data[$result['language_id']][$result['key']] = $result['value'];
		}

		return $store_config_data;
	}
