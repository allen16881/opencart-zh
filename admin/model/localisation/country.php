	...
	public function getCountries($data = array()) {
		if ($data) {
			...
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				$sql .= $data['sort'] == 'name' ? " ORDER BY sort_order DESC, name" : " ORDER BY " . $data['sort'];
			} else {
				$sql .= " ORDER BY sort_order DESC, name";
			}
			...
			if (!$country_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "country ORDER BY sort_order DESC, name ASC");
