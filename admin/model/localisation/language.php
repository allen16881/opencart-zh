...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		// Setting
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $setting) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET store_id = '" . (int)$setting['store_id'] . "', language_id = '" . (int)$language_id . "', code = '" . $this->db->escape($setting['code']) . "', `key` = '" . $this->db->escape($setting['key']) . "', value = '" . $this->db->escape($setting['value']) . "', serialized = '" . (int)$setting['serialized'] . "'");
		}
		...
		foreach ($query->rows as $seo_url) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET store_id = '" . (int)$seo_url['store_id'] . "', language_id = '" . (int)$language_id . "', query = '" . $this->db->escape($seo_url['query']) . "', keyword = '" . $this->db->escape($seo_url['keyword']) . "'");
		}
