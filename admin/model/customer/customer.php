...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				$address['firstname'] = isset($address['firstname']) ? $address['firstname'] : '';
				$address['lastname'] = isset($address['lastname']) ? $address['lastname'] : '';
				$address['fullname'] = isset($address['fullname']) ? $address['fullname'] : '';
				$address['telephone'] = isset($address['telephone']) ? $address['telephone'] : '';
				$address['company'] = isset($address['company']) ? $address['company'] : '';
				$address['address_2'] = isset($address['address_2']) ? $address['address_2'] : '';
				$address['city'] = isset($address['city']) ? $address['city'] : '';

				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', fullname = '" . $this->db->escape($address['fullname']) . "', telephone = '" . $this->db->escape($address['telephone']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "', city_id = '" . (int)$address['city_id'] . "', district_id = '" . (int)$address['district_id'] . "', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? json_encode($address['custom_field']) : json_encode(array())) . "'");
	...
	public function getTotalCustomers($data = array()) {
		...
		if (!empty($data['filter_fullname'])) {
			$implode[] = "fullname LIKE '" . $this->db->escape($data['filter_fullname']) . "%'";
		}
