...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		} elseif (isset($this->session->data['filemanager_directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/' . str_replace('*', '', $this->session->data['filemanager_directory']), '/');
		} else {
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'type'  => 'image',
					'path'  => utf8_substr($image, utf8_strlen(DIR_IMAGE)),
					'href'  => $server . 'uploads/' . utf8_substr($image, utf8_strlen(DIR_IMAGE))
				);
			}
		}
		...
		if (isset($this->request->get['directory'])) {
			...
			$pos = strrpos($this->session->data['filemanager_directory'], '/');

			if ($pos) {
				$url .= '&directory=' . urlencode(substr($this->session->data['filemanager_directory'], 0, $pos));
			} else {
				$url .= '&directory=%2F';
			}
		}
		...
		if (!$json) {
			...
			foreach ($files as $file) {
				if (is_file($file['tmp_name'])) {
					...
					if ((utf8_strlen($filename) < 1) || (utf8_strlen($filename) > 255)) {
