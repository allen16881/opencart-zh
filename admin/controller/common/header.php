...
class ControllerCommonHeader extends Controller {
	public function index() {
		if (isset($this->session->data['user_token'])) {
			$this->document->addValue('user_token', $this->session->data['user_token']);
		}
		...
		$data['page_title_append'] = $this->language->get('page_title_append');
		...
		if (!isset($this->request->get['user_token']) || !isset($this->session->data['user_token']) || ($this->request->get['user_token'] != $this->session->data['user_token'])) {
			...
			if ($user_info) {
				...
				if (is_file(DIR_IMAGE . $user_info['image'])) {
					$data['image'] = $this->model_tool_image->resize($user_info['image'], 100, 100);
				} else {
					$data['image'] = $this->model_tool_image->resize('no_avatar.png', 100, 100);
				}
			} else {
				$data['firstname'] = '';
				$data['lastname'] = '';
