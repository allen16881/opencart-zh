...
class ControllerCatalogReview extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['parent_id'])) {
				$this->response->redirect($this->url->link('catalog/review/edit', 'user_token=' . $this->session->data['user_token'] . '&review_id=' . $this->request->post['parent_id'] .$url, true));
			} else {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->response->redirect($this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . $url, true));
		} else if (isset($this->request->get['review_id']) && $this->validateDelete()) {
			...
			if (!empty($review_info['parent_id'])) {
				$this->response->redirect($this->url->link('catalog/review/edit', 'user_token=' . $this->session->data['user_token'] . '&review_id=' . $review_info['parent_id'], true));
			} else {
	...
	protected function getList() {
		...
		$data['sort_featured'] = $this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . '&sort=r.featured' . $url, true);
		$data['sort_replies'] = $this->url->link('catalog/review', 'user_token=' . $this->session->data['user_token'] . '&sort=replies' . $url, true);
	...
	protected function getForm() {
		...
		$data['text_form'] = isset($this->request->get['parent_id']) ? $this->language->get('text_reply') : $data['text_form'];
		...
		if (!isset($this->request->get['review_id'])) {
			...
			$filter_data = array(
				'filter_parent_id'  => $this->request->get['review_id'],
				'sort'  => 'r.date_added',
				'order'  => 'DESC'
			);

			$data['replies'] = $this->model_catalog_review->getReviews($filter_data);

			foreach ($data['replies'] as $key => $reply) {
				$data['replies'][$key]['edit'] = $this->url->link('catalog/review/edit', 'user_token=' . $this->session->data['user_token'] . '&review_id=' . $reply['review_id'] . $url, true);
				$data['replies'][$key]['delete'] = $this->url->link('catalog/review/delete', 'user_token=' . $this->session->data['user_token'] . '&review_id=' . $reply['review_id'] . $url, true);
			}
		}
		...
		if (!empty($review_info) && empty($review_info['parent_id'])) {
			$data['add'] = $this->url->link('catalog/review/add', 'user_token=' . $this->session->data['user_token'] . '&parent_id=' . $this->request->get['review_id'] . $url, true);
		}

		if (!empty($review_info) && !empty($review_info['parent_id'])) {
			$data['cancel'] = $this->url->link('catalog/review/edit', 'user_token=' . $this->session->data['user_token'] . '&review_id=' . $review_info['parent_id'] . $url, true);

			$data['parent_info'] = $this->model_catalog_review->getReview($review_info['parent_id']);
		}

		if (isset($this->request->get['parent_id'])) {
			$data['parent_info'] = $this->model_catalog_review->getReview($this->request->get['parent_id']);
		}

		if (isset($this->request->post['parent_id'])) {
			$data['parent_info'] = $this->model_catalog_review->getReview($this->request->post['parent_id']);
		}
		...
		$data['user_name'] = empty($user_info['fullname']) ? $user_info['username'] : $user_info['fullname'];
		...
		if (isset($this->request->post['featured'])) {
			...
			$data['featured'] = $review_info['featured'];
		} else {
			$data['featured'] = '';
		}
