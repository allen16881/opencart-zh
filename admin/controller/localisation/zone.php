...
class ControllerLocalisationZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			} else if (isset($this->request->get['country_id'])) {
				$url .= '&country_id=' . $this->request->get['country_id'];
			}
	...
	protected function getList() {
		...
		if (isset($this->request->get['parent_id'])) {
			...
			$zone_info = $this->model_localisation_zone->getZone($this->request->get['parent_id']);

			if ($zone_info) {
				$country_id = $zone_info['country_id'];

				$this->document->setTitle($this->language->get('heading_title').'-'.$zone_info['name']);

				$data['heading_title'] = $this->language->get('heading_title').'-'.$zone_info['name'];
			}

			$zone_total = $this->model_localisation_zone->getTotalZonesByParentId($this->request->get['parent_id']);
		} else if (isset($this->request->get['country_id'])) {
			$url .= '&country_id=' . $this->request->get['country_id'];
			...
			if ($country_info) {
				$this->document->setTitle($this->language->get('heading_title').'-'.$country_info['name']);

				$data['heading_title'] = $this->language->get('heading_title').'-'.$country_info['name'];
			}

			$zone_total = $this->model_localisation_zone->getTotalZonesByCountryId($this->request->get['country_id']);
		} else {
		...
		foreach ($results as $result) {
			$data['zones'][] = array(
				...
				'list'    => $this->url->link('localisation/zone', 'user_token=' . $this->session->data['user_token'] . '&parent_id=' . $result['zone_id'], true)
			);
		}
		...
		$data['sort_sort_order'] = $this->url->link('localisation/zone', 'user_token=' . $this->session->data['user_token'] . '&sort=z.sort_order' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->get['parent_id'])) {
			...
			$data['parent_info'] = $this->model_localisation_zone->getZone($this->request->get['parent_id']);
		}
		...
		if (isset($this->request->get['country_id'])) {
			...
			$data['country_info'] = $this->model_localisation_country->getCountry($this->request->get['country_id']);
		}
		...
		if (isset($this->request->post['parent_id'])) {
			...
			$data['parent_id'] = $zone_info['parent_id'];
		} else {
			$data['parent_id'] = '';
		}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $zone_id) {
			...
			$zone_total = $this->model_localisation_zone->getTotalZonesByParentId($zone_id);
