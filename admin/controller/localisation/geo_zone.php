...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function edit() {
		...
		$this->document->addValue('text_select', $this->language->get('text_all_zones'));
		$this->document->addValue('text_none', $this->language->get('text_none'));
	...
	protected function getForm() {
		...
		foreach ($data['zone_to_geo_zones'] as $key => $value) {
			if (!empty($value['country_id'])) {
				$data['zone_to_geo_zones'][$key]['zones'] = $this->model_localisation_zone->getZonesByCountryId($value['country_id']);
			}

			if (!empty($value['zone_id'])) {
				$data['zone_to_geo_zones'][$key]['cities'] = $this->model_localisation_zone->getZonesByParentId($value['zone_id']);
			}

			if (!empty($value['city_id'])) {
				$data['zone_to_geo_zones'][$key]['districts'] = $this->model_localisation_zone->getZonesByParentId($value['city_id']);
			}
		}
