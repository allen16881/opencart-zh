...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		$data['sort_fullname'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.fullname' . $url, true);
...
*/
		...
		foreach ($data['addresses'] as $key => $value) {
			$data['addresses'][$key]['zones'] = $this->model_localisation_zone->getZonesByCountryId($value['country_id']);
			$data['addresses'][$key]['cities'] = $value['zone_id'] ? $this->model_localisation_zone->getZonesByParentId($value['zone_id']) : '';
			$data['addresses'][$key]['districts'] = $value['city_id'] ? $this->model_localisation_zone->getZonesByParentId($value['city_id']) : '';
		}
...
*/
				...
				if ((utf8_strlen($value['address_1']) < 1) || (utf8_strlen($value['address_1']) > 128)) {
...
/*				if ((utf8_strlen($value['city']) < 1) || (utf8_strlen($value['city']) > 128)) {
				...
				if (isset($value['country_id']) && !$value['country_id'] > 0) {
					$zones = $this->model_localisation_zone->getZonesByCountryId($value['country_id']);

					if ($zones && (!isset($value['zone_id']) || !$value['zone_id'])) {
						$this->error['address'][$key]['zone'] = $this->language->get('error_zone');
					}
				}

				if (isset($value['zone_id']) && $value['zone_id'] > 0) {
					$cities = $this->model_localisation_zone->getZonesByParentId($value['zone_id']);

					if ($cities && (!isset($value['city_id']) || !$value['city_id'])) {
						$this->error['address'][$key]['city'] = $this->language->get('error_city');
					}
				}

				if (isset($value['city_id']) && $value['city_id'] > 0) {
					$districts = $this->model_localisation_zone->getZonesByParentId($value['city_id']);

					if ($districts && (!isset($value['district_id']) || !$value['district_id'])) {
						$this->error['address'][$key]['district'] = $value('error_district');
					}
				}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_fullname']) || isset($this->request->get['filter_email']) || isset($this->request->get['filter_telephone'])) {
			if (isset($this->request->get['filter_name'])) {
				$filter_name = $this->request->get['filter_name'];
			} else {
				$filter_name = '';
