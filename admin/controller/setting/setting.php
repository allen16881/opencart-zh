...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['store_config'])) {
			$data['store_config'] = $this->request->post['store_config'];
		} else {
			$data['store_config'] = $this->model_setting_store->getStoreConfigs(0);
		}

		$this->load->model('tool/image');

		foreach ($data['store_config'] as $language_id => $value_data) {
			if (isset($value_data['config_image']) && !empty($value_data['config_image']) && is_file(DIR_IMAGE . $value_data['config_image'])) {
				$data['store_config'][$language_id]['thumb'] = $this->model_tool_image->resize($value_data['config_image'], 100, 100);
			} else {
				$data['store_config'][$language_id]['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
			}
		}
		...
		if (isset($this->request->post['config_district_id'])) {
			$data['config_district_id'] = $this->request->post['config_district_id'];
		} else {
			$data['config_district_id'] = $this->config->get('config_district_id');
		}

		if (isset($this->request->post['config_sales_country_id'])) {
			$data['config_sales_country_id'] = $this->request->post['config_sales_country_id'];
		} else {
			$data['config_sales_country_id'] = $this->config->get('config_sales_country_id');
		}

		if (isset($this->request->post['config_sales_zone_id'])) {
			$data['config_sales_zone_id'] = $this->request->post['config_sales_zone_id'];
		} else {
			$data['config_sales_zone_id'] = $this->config->get('config_sales_zone_id');
		}

		if (isset($this->request->post['config_sales_city_id'])) {
			$data['config_sales_city_id'] = $this->request->post['config_sales_city_id'];
		} else {
			$data['config_sales_city_id'] = $this->config->get('config_sales_city_id');
		}

		if (isset($this->request->post['config_sales_district_id'])) {
			$data['config_sales_district_id'] = $this->request->post['config_sales_district_id'];
		} else {
			$data['config_sales_district_id'] = $this->config->get('config_sales_district_id');
		}
		...
		if (!empty($data['config_country_id'])) {
			$data['zones'] = $this->model_localisation_zone->getZonesByCountryId($data['config_country_id']);
		}

		if (!empty($data['config_zone_id'])) {
			$data['cities'] = $this->model_localisation_zone->getZonesByParentId($data['config_zone_id']);
		}

		if (!empty($data['config_city_id'])) {
			$data['districts'] = $this->model_localisation_zone->getZonesByParentId($data['config_city_id']);
		}

		if (!empty($data['config_sales_country_id'])) {
			$data['sales_zones'] = $this->model_localisation_zone->getZonesByCountryId($data['config_sales_country_id']);
		}

		if (!empty($data['config_sales_zone_id'])) {
			$data['sales_cities'] = $this->model_localisation_zone->getZonesByParentId($data['config_sales_zone_id']);
		}

		if (!empty($data['config_sales_city_id'])) {
			$data['sales_districts'] = $this->model_localisation_zone->getZonesByParentId($data['config_sales_city_id']);
		}
		...
		if (isset($this->request->post['config_shopping_faq_id'])) {
			$data['config_shopping_faq_id'] = $this->request->post['config_shopping_faq_id'];
		} else {
			$data['config_shopping_faq_id'] = $this->config->get('config_shopping_faq_id');
		}
		...
		if (isset($this->request->post['config_product_comment_status'])) {
			$data['config_product_comment_status'] = $this->request->post['config_product_comment_status'];
		} else {
			$data['config_product_comment_status'] = $this->config->get('config_product_comment_status');
		}

		if (isset($this->request->post['config_product_comment_guest'])) {
			$data['config_product_comment_guest'] = $this->request->post['config_product_comment_guest'];
		} else {
			$data['config_product_comment_guest'] = $this->config->get('config_product_comment_guest');
		}
		...
		$data['captcha_pages'][] = array(
			'text'  => $this->language->get('text_product_comment'),
		...
		$data['mail_alerts'][] = array(
			'text'  => $this->language->get('text_mail_comment'),
			'value' => 'comment'
		);
	...
	protected function validate() {
		...
		foreach ($this->request->post['store_config'] as $language_id => $value) {
			if (!$value['config_name']) {
				$this->error['name'][$language_id] = $this->language->get('error_name');
			}

			/*if ((utf8_strlen($value['config_owner']) < 1) || (utf8_strlen($value['config_owner']) > 64)) {
				$this->error['owner'][$language_id] = $this->language->get('error_owner');
			}

			if ((utf8_strlen($value['config_address']) < 1) || (utf8_strlen($value['config_address']) > 256)) {
				$this->error['address'][$language_id] = $this->language->get('error_address');
			}

			if ((utf8_strlen($value['config_email']) > 96) || !filter_var($value['config_email'], FILTER_VALIDATE_EMAIL)) {
				$this->error['email'][$language_id] = $this->language->get('error_email');
			}

			if ((utf8_strlen($value['config_telephone']) < 1) || (utf8_strlen($value['config_telephone']) > 32)) {
				$this->error['telephone'][$language_id] = $this->language->get('error_telephone');
			}
	...
	public function theme() {
		...
		if (null != DIR_TEMPLATE_CATALOG) {
			$theme_folder = str_replace(DIR_MAIN, '', DIR_TEMPLATE_CATALOG);

			$theme_dir = DIR_TEMPLATE_CATALOG;
		} else {
			$theme_folder = 'themes/';

			$theme_dir = DIR_MAIN . $theme_folder;
		}

		if (is_file($theme_dir . $theme . '/icon.png')) {
			$this->response->setOutput($server . $theme_folder . $theme . '/icon.png');
		} else {
			$this->response->setOutput($server . 'uploads/no_image.png');
		}
	}
}
