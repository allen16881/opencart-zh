...
class ModelUpgrade1000 extends Model {
	public function upgrade() {
		...
		foreach($lines as $line) {
			// Set any prefix
			$line = str_replace("DROP TABLE IF EXISTS `cnoc_", "DROP TABLE IF EXISTS `" . DB_PREFIX, $line);

			$line = str_replace("CREATE TABLE IF NOT EXISTS `cnoc_", "CREATE TABLE `" . DB_PREFIX, $line);

			$line = str_replace("CREATE TABLE `cnoc_", "CREATE TABLE `" . DB_PREFIX, $line);
		...
		foreach ($table_new_data as $table) {
			...
			if (!isset($table_old_data[$table['name']])) {
				...
				// Loop through all tables and adjust based on cnoc.sql file
				$i = 0;
