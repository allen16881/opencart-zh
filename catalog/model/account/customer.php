...
class ModelAccountCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET fullname = '" . $this->db->escape($data['fullname']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");//firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "',
	}
	...
	public function getAvatar($customer_id) {
		$query = $this->db->query("SELECT avatar FROM `" . DB_PREFIX . "customer` WHERE customer_id = '" . (int)$customer_id . "'");

		return $query->row['avatar'];
	}

	public function editAvatar($avatar) {
		$query = $this->db->query("SELECT avatar FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$this->customer->getId() . "'");

		$old = $query->row['avatar'];

		if ($old) {
			@unlink(DIR_IMAGE . $old);
		}
