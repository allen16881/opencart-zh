...
class ControllerAccountAddress extends Controller {
	...
	protected function getList() {
		...
		$data['profile'] = $this->url->link('account/edit', '', true);
		$data['password'] = $this->url->link('account/password', '', true);
		$data['address'] = $this->url->link('account/address', '', true);
		$data['newsletter'] = $this->url->link('account/newsletter', '', true);
		$data['avatar'] = $this->url->link('account/avatar', '', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['address_id'])) {
			...
			$data['remove_action'] = $this->url->link('account/address/delete', 'address_id=' . $this->request->get['address_id'], true);
		}
		...
		if (isset($this->request->post['country_id'])) {
			...
			$data['country_id'] = $this->config->get('config_sales_country_id') ? $this->config->get('config_sales_country_id') : $this->config->get('config_country_id');
		}
		...
		if ($this->config->get('config_sales_country_id') && ($this->config->get('config_sales_country_id') == $data['country_id'] || empty($data['country_id']))) {
			$data['sales_country_id'] = $this->config->get('config_sales_country_id');

			$data['country_id'] = $data['sales_country_id'];
		}

		if ($this->config->get('config_sales_zone_id') && ($this->config->get('config_sales_zone_id') == $data['zone_id'] || empty($data['zone_id']))) {
			$data['sales_zone_id'] = $this->config->get('config_sales_zone_id');

			$data['zone_id'] = $data['sales_zone_id'];
		}

		if ($this->config->get('config_sales_city_id') && ($this->config->get('config_sales_city_id') == $data['city_id'] || empty($data['city_id']))) {
		...
		if (isset($data['zones']) && !empty($data['zones'])) {
			foreach ($data['zones'] as $zone) {
				if ($zone['zone_id'] == $data['zone_id']) {
					$data['address_area'] .= $zone['name'];
				}
			}
		}
		if (isset($data['cities']) && !empty($data['cities'])) {
			foreach ($data['cities'] as $city) {
				if ($city['zone_id'] == $data['city_id']) {
					$data['address_area'] .= $city['name'];
				}
			}
		}
		if (isset($data['districts']) && !empty($data['districts'])) {
			foreach ($data['districts'] as $district) {
				if ($district['zone_id'] == $data['district_id']) {
					$data['address_area'] .= $district['name'];
				}
			}
		}
