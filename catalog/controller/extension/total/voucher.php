...
class ControllerExtensionTotalVoucher extends Controller {
	...
	public function voucher() {
		...
		if (empty($this->request->post['voucher'])) {
			$json['error']['message'] = $this->language->get('error_empty');
			...
			$json['error']['message'] = $this->language->get('error_voucher');
			...
			if ($voucher_query->num_rows) {
				...
				foreach ($voucher_query->rows as $voucher) {
					...
					if (is_file(DIR_IMAGE . $voucher['image'])) {
						$data['image'] = $this->config->get('config_url') . 'uploads/' . $voucher['image'];
					} else {
						$data['image'] = '';
					}
