...
class ControllerStartupStartup extends Controller {
	public function index() {
		...
		$language = new Language($code, $this->config->get('config_theme'));
		...
		if ($languages[$code]['language_id']) {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "setting` WHERE (store_id = '0' OR store_id = '" . (int)$this->config->get('config_store_id') . "') AND language_id = '" . (int)$languages[$code]['language_id'] . "' ORDER BY store_id ASC");
		...
		if ($theme != $this->config->get('config_theme')) {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "setting` WHERE `key` = 'theme_" . $this->db->escape($theme) . "_status' AND `value` = '1'");

			if ($query->rows) {
				$this->config->set('config_theme', $theme);
			...
			setcookie('theme', '', time() + 3600 * 24 * 30, '/');
		}

		if ($theme == 'mobile') {
			$this->config->set('config_seo_url', '2');
		}
